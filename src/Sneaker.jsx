...
function Sneaker() {
  const sneakers = [
    ...
    },
    {
      ...
      price: 200,
      sizes: [40, 41, 42, 43],
      colors: ["Orange", "White", "Black","Blue","Red"],
    },
    {
      ...
      price: 30,
      sizes: [40, 41, 42, 43],
      colors: ["Orange", "White","Blue" ,"Black"],
    },
    {
  ...
  const handleBuyToggle = (index) => {
    const updatedSneakers = [...filteredSneakers];
    updatedSneakers[index].isAdded = !updatedSneakers[index].isAdded;
    ...
    <>
      <div className='my-20 sm:mb-[10rem] mb-[22rem] text-center text-2xl text-slate-900 Sneaker '>
        ...
        <div className='flex w-fit m-auto border-2 gap-4 rounded-xl  bg-[#E4F1FF]  p-1'>
          ...
          <button className='m-auto rounded-lg  p-2 hover:bg-teal-100 hover:text-slate-950' onClick={applyFilter}><img src="/search_.svg" alt="" /></button>
        </div>
          ...
          {filteredSneakers.map((sneaker, index) => (
            <div key={index} className='z-20 w-fit shadow-lg rounded-xl snker p-0 hover:shadow-teal-300 m-auto hover:translate-y-[-0.4rem] transition-all ease-in-out hover:transition-all hover:ease-in-out '>
              ...
              <div className='flex w-full m-auto '>
                <select name='Size' className='m-auto  w-full bg-[#EEF7FF] border-y-4 border-teal-900  p-2 hover:cursor-pointer  transition-all ease-in-out '>{"Size"}
                  {sneaker.sizes.map((size, sizeIndex) => (
                    <option className='text-slate-950 text-center ' key={sizeIndex} value={size}>{size}</option>
                  ))}
                </select>
                <select name='Color' className='m-auto  w-full hover:cursor-pointer border-y-4 border-teal-900   transition-all ease-in-out bg-[#F3F8FF]   p-2'>{"Color"}
              ...
              <button onClick={() => handleBuyToggle(index)} className=' hover:text-slate-50 rounded-b-lg p-2 text-3xl m-auto w-full transition-all ease-in-out hover:bg-[#053B50] bg-slate-50'>{sneaker.isAdded ? 'Added'  : 'Buy'}</button>
            </div>
          ))}
        </div>
      </div>
